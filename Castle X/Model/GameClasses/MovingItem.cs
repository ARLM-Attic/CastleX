...
namespace CastleX
{
    ...
    public class MovingItem   
    {
        ...
        /// <summary>   
        /// Gets whether or not the player's feet are on the moving item 
        /// </summary>   
        ...
        public MovingItem(Texture2D texture, ScreenManager ThisScreenManager, Level level, Vector2 position)   
        {
        ...
        {   
            ...
            {
                // If we are about to run into a wall that is not a movingItem move in other direction.   
            ...
            if (level.MovingItems.Count > 0)   
            {   
                // If we are about to run into a moving item move in other direction.   
                foreach (var movingItem in level.MovingItems)   
                {   
                    if (BoundingRectangle != movingItem.BoundingRectangle)   
                    {   
                        if (BoundingRectangle.Intersects(movingItem.BoundingRectangle))   
                        {   
                            direction = (FaceDirection)(-(int)direction);   
                            velocity = new Vector2((int)direction * MoveSpeed * elapsed, 0.0f);   
                        }   
